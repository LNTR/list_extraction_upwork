...
empty_row_placeholder_word_list=["LJ  UL.","E  UL."]

df = pd.DataFrame(columns = df_columns)


BASE_URL="https://www.advokatskakomora.me"
URL  =  f"{BASE_URL}/PODGORICA_LJ.html"
...
def get_div_list_without_empty_rows(soup):
    ...
    for div in div_list_with_empty_rows:
        if not(div_has_keywords(div)):
            div_list.append(div)
        
    return div_list


def div_has_keywords(div):
    for empty_row_placeholder_word in empty_row_placeholder_word_list:
        if empty_row_placeholder_word in div.text.strip():
            return True
    return False
